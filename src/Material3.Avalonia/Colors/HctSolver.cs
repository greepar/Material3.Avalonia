...
public static class HctSolver
{
    ...
    {
        var scaledDiscount =
            MathUtils.MatrixMultiply(linrgb, ScaledDiscountFromLinrgb);
    ...
    /// R=coordinate, G=coordinate, or B=coordinate</returns>
    ...
    private static double InverseChromaticAdaptation(double adapted)
    {
        ...
        var base_ = Math.Max(0, 27.13 * adaptedAbs / (400.0 - adaptedAbs));
        return MathUtils.Signum(adapted) * Math.Pow(base_, 1.0 / 0.42);
    }
    ...
    /// <returns>The desired color as an ARGB integer, if found; 0 otherwise.</returns>
    private static uint FindResultByJ(double hueRadians, double chroma, double y)
    {
        ...
        for (var iterationRound = 0; iterationRound < 5; iterationRound++)
        {
            ...
            var linrgb = MathUtils.MatrixMultiply(
                new[] { rCScaled, gCScaled, bCScaled },
    ...
    /// <returns>An ARGB integer representing the sRGB color. The color
    ...
    /// <param name="chroma">The desired chroma.</param>
    /// <param name="lstar">The desired L*.</param>
    /// <returns>A CAM16 object representing the sRGB color. The color
    /// has sufficiently close hue, chroma, and L* to the desired
    /// values, if possible; otherwise, the hue and L* will be
    /// sufficiently close, and chroma will be maximized.</returns>
